    ...
    def test_If_signed_suspicious(self):
        ...
        with self.assertWarnsRegex(SyntaxWarning,
                (r"^Signed values in If\/Elif conditions usually result from inverting Python "
                    r"booleans with ~, which leads to unexpected results: ~True is -2, which is "
                    r"truthful\. Replace `~flag` with `not flag`\. \(If this is a false positive, "
                    r"silence this warning with `m\.If\(x\)` → `m\.If\(x\.boo\l\(\)\)`\.\)$")):
            with m.If(~True):
                pass
    ...
    def test_Elif_signed_suspicious(self):
        ...
        with self.assertWarnsRegex(SyntaxWarning,
                (r"^Signed values in If\/Elif conditions usually result from inverting Python "
                    r"booleans with ~, which leads to unexpected results: ~True is -2, which is "
                    r"truthful. Replace `~flag` with `not flag`\. \(If this is a false positive, "
                    r"silence this warning with `m\.If\(x\)` → `m\.If\(x.bool\(\)\)`\.\)$")):
            with m.Elif(~True):
                pass
